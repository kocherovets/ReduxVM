...
- **State** хранит текущее состояние приложения и позволяет на себя подписаться чтобы отслеживать изменения. В ReduxVM к модулю State подключаются модули Presenter и Interactor, но можно и непосредственно подписаться на State, если это понадобится. State представляет собой swift структуру, которая обычно состоит из нескольких подструктур описывающих разные разделы бизнес логики. 
...
- **Interactor** представляет собой подписчиков к Store не связанных напрямую с UI, то есть код Interactor выполняется в фоновом потоке. Interactor является управляющей сущностью для некоторого набора SideEffects обычно логически связанных с точки зрения программы. Например, это может быть интерактор для работы с сетью. При изменении State каждый Interactor опрашивает свои SideEffects узнавая должны ли они запуститься и если должны, то запускает.
...
- В настоящее время нет хорошего способа обработки Cross CuttingConcern в Swift. В ReduxVM, вы получите его бесплатно. Вы можете решать различные задачи, используя middleware и интеракторы. Это позволяет легко справиться с такими задачами как ведением журнала или статистикой.
...
### ThrottleAction
Из практики использования библиотеки оказалось удобно ввести специальный тип Action, который не должен повторно обрабатываться системой в течении какого-то времени. Например, это удобно для избежания повторного срабатывания нажатия на кнопку, когда это не нужно. Для этого в библиотеке есть протокол ThrottleAction
```swift
...
```
В библиотеке есть реализация по умолачанию LoggingMiddleware. По умолчаниию все Action в системе логируются, для некоторых из них бывает удобно отключить логирование, в loggingExcludedActions перечисляются такие Action. В параметре firstPart задается элемент пути с конца строки, с которого будет логироваться имя файла. Например, для полного пути A/B/C/C/D, если задать firstPart равным C, то будет выведено C/D.
```swift
...
## Interactor
При создании интерактора нужно перечислить сайдэффекты, которые он обслуживает. 
```swift
...
```func execute(box: StateBox<State>, trunk: Trunk, interactor: APIInteractor) {``` - это собственно рабочая часть сайдэффекта. На вход помимо StateBox передается trunk, чтобы сайдэффект мог по результатам своей деятельности поменять стейт, а также ссылка на интерактор, к которому принадлежит этот сайдэффект. 
...
У каждого сайдэффекта также есть два поля 
```swift
    var queue: DispatchQueue? { nil }
    var async: Bool { true }
```
С помощью них можно указать отдельную очередь для выполнения этого сайдэффекта, а также выполнять ли его асинхронно. По умолчанию очередь не задана и выполнения сайдэффекта проихсодит как на рисунке в основной фоновой очереди ReduxVM. С помощью этой настройки можно удобно организовать всю работу с каким-либо сервисом в отдельной очереди, создав базовый протокол с нужной очередью для всех таких сайдэффектов. Это можеть быть полезно, если сервис выполняет какие-то тяжелые операции, и нежелательно, чтобы он работал в основной очереди. Пример можно посмотреть по [ссылке.](https://github.com/kocherovets/RedSwift/blob/master/RedSwift/AsyncIntercator2.swift)
# Use Cases
